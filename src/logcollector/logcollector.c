...
int w_msg_hash_queues_push(const char *str,char *file,unsigned long size,logtarget * log_target,char queue_mq){
    w_msg_queue_t *msg;
    int i;

    for (i = 0; log_target[i].log_socket && log_target[i].log_socket->name; i++)
    {
        w_mutex_lock(&mutex);

        while (msg = (w_msg_queue_t *)OSHash_Get(msg_queues_table,log_target[i].log_socket->name), !msg) {
            w_cond_wait(&available, &mutex);
        }

        w_mutex_unlock(&mutex);

        w_msg_queue_push(msg,str,file,size,log_target,queue_mq);
        return 0;
    }
...
void * w_output_thread(void * args){
    ...
    while(1)
    {
        ...
        if (SendMSGtoSCK(logr_queue, message->buffer, message->file,
                        message->queue_mq, message->log_target) < 0) {
                merror(QUEUE_SEND);
